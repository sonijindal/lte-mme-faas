...
func main() {
	...
	file := "./test/results/enb_app_aws_concur_cass" + "-" + strconv.Itoa(async) + "-" + time.Now().Format("2006.01.02-15:04:05")
	fmt.Println(file)
	var err error
	...
	go func() {
		for {
			...
			case job := <-w.JobChannel:
				//fmt.Println("we have received a work request")
